...
class MQTTClientGUI:
    def __init__(self, root):
        ...
        self.publish_button = ttk.Button(pub_frame, text="Publish", command=self.publish_messages, state="disabled")
        self.publish_button.grid(row=0, column=3, padx=5, pady=5)
    ...
    def send_message(self):
        if not self.client or not self.is_connected:
            self.messages_text.insert(tk.END, "Not connected to broker!\n")
            return
    ...
    async def publish_file_content(self):
        if not hasattr(self, 'selected_file'):
            self.messages_text.insert(tk.END, "Please select a file first!\n")
            return
    ...
    def publish_messages(self):
        asyncio.run_coroutine_threadsafe(self.publish_file_content(), self.loop)
