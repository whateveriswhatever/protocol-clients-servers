...
class MQTTClientGUI:
    def __init__(self, root):
        ...
        self.format_var.trace('w', self.on_format_change)  # Add trace to format variable
    ...
    def on_format_change(self, *args):
        ...
        self.messages_text.insert(tk.END, f"Switched to {self.format_var.get()} format. Please select a new file.\n")
    ...
    def validate_file_format(self, filename):
        """Validate if the selected file matches the chosen format"""
        file_ext = os.path.splitext(filename)[1].lower()
        chosen_format = self.format_var.get().lower()
        
        if chosen_format == 'json' and file_ext != '.json':
            return False
        elif chosen_format == 'csv' and file_ext != '.csv':
            return False
        return True

    def select_file(self):
        current_format = self.format_var.get()
        file_types = [('JSON files', '*.json')] if current_format == "JSON" else [('CSV files', '*.csv')]
